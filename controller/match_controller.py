...
from view import player_view, menus_view
from controller import player_controller, writer_data_controller, round_controller
...
class MatchDeserializer:
    ...
    def create_pairs_players(tournament_name):
        ...
        first_list = list_players_sort_name[: (len(list_players_sort_name) // 2)]
        second_list = list_players_sort_name[(len(list_players_sort_name) // 2) :]
    ...
    def check_and_change_order_list_players(tournament_name, round_name):
        first_list, second_list = MatchDeserializer.id_pair_players(tournament_name)
        ...
        rep_match_list = [x for x in old_matches_and_inverse if x in new_matches]
        try:
            rep_match_list = [x for x in old_matches_and_inverse if x in new_matches]
            rep_match = []
            ...
            while rep_match_list and repeat != 7:
                ...
                ):  # si le match repeté est en dernier, alors on le place en 3eme position
                    new_matches[2], new_matches[3] = new_matches[3], new_matches[2]
...
class MatchesSerializer:
    ...
    def matches_serializer(tournament_name, round_name):
        serialized_matches = []
        data_matches = CreateMatch().create_matches_round(tournament_name, round_name)
