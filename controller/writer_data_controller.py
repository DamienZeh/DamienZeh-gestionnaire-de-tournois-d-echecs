...
def writer_data_tournament():
    """It gets tournament_serializer and write it in
    tournament_table in json file.
    Returns name
    """
    ...
    names_list = \
        tournament_controller.TournamentDeserializer.all_tournaments_name()
...
def writer_data_start_round(tournament_name, round_name):
    """It gets start_round_serializer and write
     it in round_table in json file.
    """
    start_round_serializer = \
        round_controller.RoundSerializer.round_start_serializer(
...
def writer_data_matches_round(tournament_name, round_name):
    """It gets matches_round_serializer and write it in
    round_table in json file.
    """
    matches_round_serializer = \
        match_controller.MatchesSerializer.matches_serializer(
            tournament_name, round_name
        )
...
def writer_data_end_round(tournament_name, round_name):
    """It gets end_round_serializer and write it in
    round_table in json file.
    """
    end_round_serializer = \
        round_controller.RoundSerializer.round_end_serializer()
    data_tournament = \
        tournament_table.search(user.nom_tournoi == f"{tournament_name}")
    id_tournament = (data_tournament[0])["id_tournament"]
    round_table.update(
...
def writer_rounds_name(tournament_name):
    """It gets rounds_name and write it in
    tournament_table(in rounds_finis) in json file.
    """
    rounds_name = \
        round_controller.RoundDeserializer.all_rounds(tournament_name)
    str_rounds_name = (
...
def writer_timestamp_start_round(tournament_name, round_name):
    """It gets timestamp and write it in
    round_table in(date_debut_round) json file.
    """
...
def writer_timestamp_end_round(tournament_name, round_name):
    """It gets timestamp and write it in
    round_table(in date_fin_round) in json file.
    """
    timestamp = tournament_controller.DataTournament.timestamp()
...
def writer_timestamp_start_tournament(tournament_name):
    """It gets timestamp and write it in
    tournament_table(in date_de_debut) in json file.
    """
    timestamp = tournament_controller.DataTournament.timestamp()
    tournament_view.\
        ShowTournament.view_timestamp_start_tournament(timestamp)
    tournament_table.update(
...
def writer_timestamp_end_tournament(tournament_name):
    """It gets timestamp and write it in
    tournament_table(in date_de_fin) in json file.
    """
    timestamp = tournament_controller.DataTournament.timestamp()
...
def writer_players_in_list(name_tournament):
    """Its gets players_serializer, and check if the new player entered
     can be added to the tournament and/or list of players existing.
    Returns no_existing = False or no_existing = True
    """
    tournament_name = name_tournament.upper().replace(" ", "_")
    players_serializer = \
        player_controller.PlayersSerializer.players_serializer()
    point_start = 0.0
...
def players_number(tournament_name):
    """It gets number_entered, players_number
     and sends them to view_enter_player"""
    number_entered = player_controller.\
        PlayersSerializer.number_players_entered(
            tournament_name
        )
